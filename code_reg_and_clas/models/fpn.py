...
def resnet_fpn(num_regions, input_height=224, input_width=224, num_heads=1):

    print('Running: ResNet 18 backbone')
    if num_heads == 1:
        print('Using single softmax head')
        model = fpn(resNet_18_encoder, num_regions, input_height=input_height, input_width=input_width)
        model.model_name = "resnet_fpn"
    
    if num_heads > 1:
        print(f'Using {num_heads} softmax heads')
        model = FPN_multiple_sfotmax_heads(resNet_18_encoder, num_regions, input_height=input_height, input_width=input_width, num_heads=num_heads)
...
def FPN_multiple_sfotmax_heads(encoder, num_regions, l1_skip_conn=True, input_height=224,
          input_width=224, num_heads=5):
    ...
    # CLASSIFICATION
    stacked_clas_output = None
    for i in range(num_heads):
        x_clas = layers.Conv2D(num_regions, (3, 3), padding='same')(y)
        output_clas = keras.activations.softmax(x_clas, axis=-1)
        
        if stacked_clas_output == None:
            stacked_clas_output = output_clas
        else:
            stacked_clas_output = layers.Concatenate(axis=-1)([stacked_clas_output, output_clas])
    
    # CONCATENATE
    output = layers.Concatenate(axis=-1)([output_masked_reg, stacked_clas_output])
